...
if (cookie) {
    ...
    container_h3.innerHTML = `<h3>В прошлый раз вы дали ${data[1]} правильных ответов из ${data[0]}>.</h3>
...
class Question {
    constructor() {
        ...
        if (sign == '+') {
            ...
            this.correct = Math.round(parseFloat((a / b)) * 100) / 100
        }
        this.answers = [
...
start_button.addEventListener('click', () => {
    ...
    setTimeout(() => {
            let new_cookie = `number_high_score=${total_answers_given}/${correct_answers_given};
            max-age=60*60*1000`
        ...
        if (total_answers_given > 0) {
            container_h3.innerHTML = `<h3>Вы дали ${correct_answers_given} правильных ответов 
        из ${total_answers_given}.</h3>
        <h3>Точность - ${Math.round((correct_answers_given * 100) / total_answers_given)}%.</h3>`
    }
         else {
            container_h3.innerHTML = `<h3>Вы дали ${correct_answers_given} правильных ответов из ${total_answers_given}.</h3>
            <h3> Точность - 0%.</h3>`
        }}, 10000)
})
